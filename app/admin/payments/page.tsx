...
export default function PaymentsPage() {
  ...
  const fetchPayments = async () => {
    try {
      ...
      // Add orderBy to sort by createdAt in descending order
      const paymentsQuery = query(
        collection(db, "payments"),
        orderBy("createdAt", "desc") // This will show newest payments first
      );
        ...
        <div className="divide-y divide-black/10 dark:divide-white/10">
          {payments
            ...
            .map((payment) => (
              <div
                key={payment.id}
                className="p-4 bg-white dark:bg-black hover:bg-black/[0.02] dark:hover:bg-white/[0.02] transition-colors"
              >
      ...
      <AlertDialog open={showRefundDialog} onOpenChange={setShowRefundDialog}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Are you sure you want to initiate a refund for payment{" "}
              {selectedPayment?.id}? Amount to be refunded: ₹
